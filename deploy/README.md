...
2. 复制 `env.example` 为 `.env`；生成各不相同的随机 `ENCRYPTION_KEY` 和 `NODE_SHARED_SECRET`。不要提交 `.env`。在国内网络可设置示例中的构建镜像源。
3. 运行 `docker compose config --quiet && docker compose up -d --build`。
4. 检查 `docker compose ps`、`curl http://127.0.0.1:8500/health`，再通过 HTTPS 入口验证网站和 `/api/projects`。当前工作台无登录门槛；公开部署时，任何知道网址的人都可调用控制面接口。
...
新的空数据库接入时，节点的 `CONTROL_URL` 指向新控制面私网地址，`NODE_SHARED_SECRET` 与服务器一致。生产环境设置 `NODE_STRICT_AUTH=1`，不接受旧控制面签发的 token；节点会先尝试旧 token，收到 401 后用共享密钥注册并保存新 token。不要把共享密钥或 token 放进 Git、截图或公开日志。Windows 节点修改常驻进程使用的配置后只重启一次，以新控制面心跳、能力列表和 probe 任务为验收依据，再停用旧控制面。后续租用 GPU 按同一方式接入。
