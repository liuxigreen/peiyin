...
## 十、提速设计账（用户点名：不光是GPU天花板）

每句18s里GPU只占一部分，流水线设计问题按浪费排序：
1. 引擎无ref缓存：同角色几百句每句重跑6-7s参考音频前端，按voice_id缓存prompt条件省1-2s/句
2. 塌缩盲试顺序反了：应默认instruct2（稳），干净refs再试其他模式，坏refs省一半GPU
3. 逐句HTTP往返：2803次claim/complete/上传(840MB家庭上行)。已实装云端侧批量认领：
   GET /me/claim?n=8（向后兼容，响应task+tasks），提交ef2c47e。
   节点接入：claim加n参数，r.json()['tasks']批量dispatch——【云端不动gpunode/entrypoint.py，
   该文件有另一agent staging血统的未提交改动，勿互踩】
4. fp16/torch.compile未开（README自己写了3060建议fp16）
5. 同角色连句无会话复用
